...
from ..utils import SimpleIndexDataset

class ScMoGCNWrapper(object):
    def __init__(self, args):
        super(ScMoGCNWrapper, self).__init__()
        self.args = args
    ...
    def predict(self, graph, idx=None):
        ...
        with torch.no_grad():
            if idx is None:
                return self.model.forward(graph)
            else:
                return self.model.forward(graph)[idx]

    def score(self, g, idx, labels):
        self.model.eval()
        with torch.no_grad():
            logits = F.relu(self.predict(g, idx))
    ...
    def fit(self, g, y, split=None, eval=True, verbose=2, y_test=None, logger=None, sampling=False):
        if sampling:
            return self.train_with_sampling(g, y, split, eval, verbose, y_test, logger)
        ...
        opt = torch.optim.AdamW(self.model.parameters(), lr=kwargs['learning_rate'], weight_decay=kwargs['weight_decay'])
        ...
        if verbose > 1:
            if eval:
                ...
                logger.write(
                    f'epoch {minvep} minimal val {minval} with training:  {tr[minvep]}\n')
            logger.close()
    ...
    def train_with_sampling(self, g, y, split=None, eval=True, verbose=2, y_test=None, logger=None):
        kwargs = vars(self.args)
        PREFIX = kwargs['prefix']
        CELL_SIZE = kwargs['CELL_SIZE']
        TRAIN_SIZE = kwargs['TRAIN_SIZE']
        ...
        for epoch in range(kwargs['epoch']):
            ...
            for i, batch_idx in enumerate(dataloader):
                ...
                #                  p=feature_weight)
                feature_sampled = torch.multinomial(feature_weight, int(0.7*len(g.nodes('feature'))), replacement=False).to(self.args.device)
                # feature_sampled = g.nodes('feature')[feature_sampled]
                subgraph = dgl.node_subgraph(g, {'cell': batch_idx.to(self.args.device), 'feature': feature_sampled})# g.nodes('feature')})
            ...
            val.append(self.score(g_origin, split['valid'], y[split['valid']]))
            ...
            if eval:
                te.append(self.score(g_origin, np.arange(TRAIN_SIZE, CELL_SIZE), y_test))
                if verbose > 1:
                    logger.write(f'testing loss:  {te[-1]}\n')
                    logger.flush()
