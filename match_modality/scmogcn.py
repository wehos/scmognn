...
class ScMoGCNWrapper(object):
    def __init__(self, args, shapes, temp=1):
        self.model = ScMoGCN(args, shapes, temp).to(args.device)
        self.args = args
    ...
    def fit(self, dataset, feats, labels):
        ...
        opt = optim.AdamW([{'params': self.model.parameters()}, {'params': wt[0]}, {'params': wt[1]}], lr=self.args.learning_rate)
    ...
    def predict(self, inputs, idx, enhance = False, dataset = None):
        ...
        with torch.no_grad():
            ...
            if not enhance:
                return self.model(m1, m2)

            else:
                emb1, emb2 = self.model.encode(m1, m2)
                matrix = batch_separated_bipartite_matching(dataset, emb1, emb2)
                return matrix
