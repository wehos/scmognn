...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--subtask', default = 'openproblems_bmmc_cite_phase2_rna', choices = ['openproblems_bmmc_cite_phase2_rna', 'openproblems_bmmc_multiome_phase2_rna'])
    ...
    # DataLoader
    dataset = ModalityMatchingDataset(subtask, data_dir=data_folder).load_data().load_sol().preprocess(pkl_path)

    if subtask == 'openproblems_bmmc_cite_phase2_rna':
        HIDDEN_SIZE = 64
        TEMPERATURE = 2.739896
        model = ScMoGCNWrapper(args, [[(dataset.preprocessed_features['mod1_train'].shape[1], 512, 0.25), (512, 512, 0.25), (512, HIDDEN_SIZE)],
                                      [(dataset.preprocessed_features['mod2_train'].shape[1], 512, 0.2), (512, 512, 0.2), (512, HIDDEN_SIZE)],
                                      [(HIDDEN_SIZE, 512, 0.2), (512, dataset.preprocessed_features['mod1_train'].shape[1])],
                                      [(HIDDEN_SIZE, 512, 0.2), (512, dataset.preprocessed_features['mod2_train'].shape[1])]], TEMPERATURE)
    else:
        HIDDEN_SIZE = 256
        TEMPERATURE = 3.065016
        model = ScMoGCNWrapper(args, [[(dataset.preprocessed_features['mod1_train'].shape[1], 1024, 0.5), (1024, 1024, 0.5), (1024, HIDDEN_SIZE)],
                                      [(dataset.preprocessed_features['mod2_train'].shape[1], 2048, 0.5), (2048, HIDDEN_SIZE)],
                                      [(HIDDEN_SIZE, 512, 0.2), (512, dataset.preprocessed_features['mod1_train'].shape[1])],
                                      [(HIDDEN_SIZE, 512, 0.2), (512, dataset.preprocessed_features['mod2_train'].shape[1])]], TEMPERATURE)
    ...
    print(model.predict(test_inputs, test_idx, True, dataset))
    print(model.score(test_inputs, test_idx,
                      z_test, True, dataset))
