...
class ModalityPredictionDataset(MultiModalityDataset):
    def __init__(self, subtask, data_dir = "./data"):
        ...
        data_url = {'openproblems_bmmc_cite_phase2_mod2': 'https://www.dropbox.com/s/snh8knscnlcq4um/openproblems_bmmc_cite_phase2_mod2.zip?dl=1',
                    ...
                    'openproblems_bmmc_multiome_phase2_rna': 'https://www.dropbox.com/s/cz60vp7bwapz0kw/openproblems_bmmc_multiome_phase2_rna.zip?dl=1'}

        super(ModalityPredictionDataset, self).__init__('predict_modality', data_url.get(subtask), subtask, data_dir)
...
class ModalityMatchingDataset(MultiModalityDataset):
    def __init__(self, subtask, data_dir = "./data"):
        ...
        data_url = {'openproblems_bmmc_cite_phase2_mod2': 'https://www.dropbox.com/s/fa6zut89xx73itz/openproblems_bmmc_cite_phase2_mod2.zip?dl=1',
                    ...
                    'openproblems_bmmc_multiome_phase2_rna': 'https://www.dropbox.com/s/h1s067wkefs1jh2/openproblems_bmmc_multiome_phase2_rna.zip?dl=1'}

        super(ModalityMatchingDataset, self).__init__('match_modality', data_url.get(subtask), subtask, data_dir)
    ...
    def preprocess(self, pkl_path=None):
        ...
        if pkl_path and (not os.path.exists(pkl_path)):
            ...
            self.preprocessed_features = {'mod1_train': m1_train, 'mod2_train': m2_train,
                                          'mod1_test': m1_test, 'mod2_test': m2_test}
...
class JointEmbeddingNIPSDataset(MultiModalityDataset):
    def __init__(self, subtask, data_dir = "./data"):
        ...
        data_url = {'openproblems_bmmc_cite_phase2': 'https://www.dropbox.com/s/hjr4dxuw55vin5z/openproblems_bmmc_cite_phase2.zip?dl=1',
                    'openproblems_bmmc_multiome_phase2': 'https://www.dropbox.com/s/40kjslupxhkg92s/openproblems_bmmc_multiome_phase2.zip?dl=1'}
        super(JointEmbeddingNIPSDataset, self).__init__('joint_embedding', data_url.get(subtask), subtask, data_dir)
        self.preprocessed = False

    def load_metadata(self):
        assert(self.loaded)
        if self.subtask.find('cite'):
            mod = 'adt'
            meta = 'cite'
        else:
            mod = 'atac'
            meta = 'multiome'
        self.exploration = [ad.read_h5ad(os.path.join(self.data_dir, self.subtask, f'{meta}_gex_processed_training.h5ad')),
                            ad.read_h5ad(os.path.join(self.data_dir, self.subtask, f'{meta}_{mod}_processed_training.h5ad')),]
        return self

    def load_sol(self):
        assert(self.loaded)
        self.test_sol = ad.read_h5ad(os.path.join(self.data_dir, self.subtask, f'{self.subtask}.censor_dataset.output_solution.h5ad'))
        return self

    def preprocess(self, pretrained_folder = '.'):

        judge_folder(pretrained_folder)
        ...
        if mod2 == "ADT":
            if os.path.exists(os.path.join(pretrained_folder, f"lsi_cite_{mod1}.pkl")):
                lsi_transformer_gex = pickle.load(open(os.path.join(pretrained_folder, f"lsi_cite_{mod1}.pkl"), "rb"))
            else:
                lsi_transformer_gex = lsiTransformer(n_components=256, drop_first=True)
                lsi_transformer_gex.fit(self.modalities[0])
                pickle.dump(lsi_transformer_gex, open(os.path.join(pretrained_folder, f"lsi_cite_{mod1}.pkl"), "wb"))

        if mod2 == "ATAC":

            if os.path.exists(os.path.join(pretrained_folder, f"lsi_multiome_{mod1}.pkl")):
                lsi_transformer_gex = pickle.load(open(os.path.join(pretrained_folder, f"lsi_multiome_{mod1}.pkl"), "rb"))
            else:
                lsi_transformer_gex = lsiTransformer(n_components=64, drop_first=True)
                lsi_transformer_gex.fit(self.modalities[0])
                pickle.dump(lsi_transformer_gex,
                        open(os.path.join(pretrained_folder, f"lsi_multiome_{mod1}.pkl"), "wb"))

            if os.path.exists(os.path.join(pretrained_folder, f"lsi_multiome_{mod2}.pkl")):
                lsi_transformer_atac = open(os.path.join(pretrained_folder, f"lsi_multiome_{mod2}.pkl"), "rb")
            else:
                #         lsi_transformer_atac = TruncatedSVD(n_components=100, random_state=random_seed)
                lsi_transformer_atac = lsiTransformer(n_components=512, drop_first=True)
                lsi_transformer_atac.fit(self.modalities[1])
                pickle.dump(lsi_transformer_atac,
                        open(os.path.join(pretrained_folder, f"lsi_multiome_{mod2}.pkl"), "wb"))
